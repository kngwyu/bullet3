...
def getRayFromTo(mouseX, mouseY):
    ...
    rayTo = [
        ...
        - float(mouseY) * dVer[0], rayFrom[1] + rayForward[1] - 0.5 * horizon[1] +
        0.5 * vertical[1] + float(mouseX) * dHor[1] - float(mouseY) * dVer[1],
        rayFrom[2] + rayForward[2] - 0.5 * horizon[2] + 0.5 * vertical[2] + float(mouseX) * dHor[2]
        - float(mouseY) * dVer[2]
    ]
...
collisionShapeId = p.createCollisionShape(
    shapeType=p.GEOM_MESH, vertices=vertices, collisionFramePosition=shift, meshScale=meshScale)
