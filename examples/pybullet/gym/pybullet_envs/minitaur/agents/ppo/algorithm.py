...
class PPOAlgorithm(object):
    ...
    """
        ...
        self._observ_filter = normalize.StreamingNormalize(
            self._batch_env.observ[0], center=True, scale=True, clip=5, name='normalize_observ')
        self._reward_filter = normalize.StreamingNormalize(
            self._batch_env.reward[0], center=False, scale=True, clip=10, name='normalize_reward')
        ...
        self._memory = memory.EpisodeMemory(template, config.update_every, config.max_length,
                                            'memory')
    ...
    """
        with tf.name_scope('experience/'):
            return tf.cond(
                self._is_training, lambda: self._define_experience(observ, action, reward), str)
    ...
    """
        with tf.name_scope('training'):
            ...
            policy_summary = self._update_policy(observ, action, old_mean, old_logstd, reward,
                                                 length)
    ...
    """
        with tf.name_scope('update_policy'):
            ...
            # pylint: disable=g-long-lambda
            loss, summary = tf.scan(
                lambda _1, _2: self._update_policy_step(observ, action, old_mean, old_logstd,
                                                        advantage, length),
    ...
    """
        ...
        loss, summary = self._policy_loss(network.mean, network.logstd, old_mean, old_logstd,
    ...
    """
        with tf.name_scope('policy_loss'):
            ...
            with tf.control_dependencies([
                    tf.cond(cutoff_count > 0, lambda: tf.Print(0, [cutoff_count], 'kl cutoff! '),
                            int)
            ]):
        ...
        with tf.name_scope('adjust_penalty'):
            ...
            with tf.control_dependencies([assert_change, print_penalty]):
                kl_change = tf.reduce_mean(
                    self._mask(
                        utility.diag_normal_kl(old_mean, old_logstd, network.mean, network.logstd),
                        length))
                ...
                maybe_increase = tf.cond(
                    ...
                    # pylint: disable=g-long-lambda
                    lambda: tf.Print(
                        self._penalty.assign(self._penalty * 1.5), [0], 'increase penalty '),
                    float)
                maybe_decrease = tf.cond(
                    kl_change < 0.7 * self._config.kl_target,
                    # pylint: disable=g-long-lambda
                    lambda: tf.Print(
                        self._penalty.assign(self._penalty / 1.5), [0], 'decrease penalty '),
                    float)
